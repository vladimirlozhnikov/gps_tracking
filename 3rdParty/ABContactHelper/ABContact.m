...
@interface CFSafePointer : NSObject
{
	CFTypeRef _pointer;
}

@property(nonatomic, readonly) CFTypeRef pointer;

+(CFSafePointer*)safePointerWithCFType:(CFTypeRef)pointer;

@end

@implementation CFSafePointer

+(CFSafePointer*)safePointerWithCFType:(CFTypeRef)pointer
{
	CFSafePointer* obj = [CFSafePointer new];
	obj->_pointer = pointer;
	CFRetain(obj->_pointer);
	return obj;
}
...
-(BOOL)isEqual:(ABContact*)object
{
	return self.recordID == object.recordID;
}
...
- (CFSafePointer*) createMultiValueFromArray: (NSArray *) anArray withType: (ABPropertyType) aType
{
	...
	CFSafePointer* pointer = [CFSafePointer safePointerWithCFType:(CFTypeRef)multi];
	CFRelease(multi);
	return pointer;
}
...
- (void) setEmailDictionaries: (NSArray *) dictionaries
{
	...
	ABMutableMultiValueRef multi = [self createMultiValueFromArray:dictionaries withType:kABMultiStringPropertyType].pointer;
...
- (void) setDateDictionaries: (NSArray *) dictionaries
{
	...
	ABMutableMultiValueRef multi = [self createMultiValueFromArray:dictionaries withType:kABMultiDateTimePropertyType].pointer;
...
- (void) setAddressDictionaries: (NSArray *) dictionaries
{
	...
	ABMutableMultiValueRef multi = [self createMultiValueFromArray:dictionaries withType:kABMultiDictionaryPropertyType].pointer;
